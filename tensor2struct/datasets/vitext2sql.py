...
@registry.register('dataset', 'vitext2sql')
class SpiderDataset(torch.utils.data.Dataset):
    def __init__(self, paths, tables_paths, db_path, demo_path=None, limit=None):
        ...
        for path in paths:
            ...
            for entry in raw_data:
                # write function preprocess raw_data: concatenate question_toks
                item = SpiderItem(
    ...
    class Metrics:
        ...
        def add(self, item, inferred_code, eval_file=None, orig_question=None):
            ret_dict = self.evaluator.evaluate_one(
                item.schema.db_id, item.orig['query_toks'], inferred_code, eval_file)
            if orig_question:
